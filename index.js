		...
		async function run() {
				...
				case "plugin":
					...
					createFileApi = `
						...
						exception when no_data_found then
							raise_application_error(-20001, 'Plugin ' || :plugin_name || ' is not valid.');
						end;`;
					break;
				default:
			...
			try {
				...
				await asyncForEach(files, async (file) => {
					...
					if (destination.toLowerCase() !== "workspace") {
					...
					if (destination.toLowerCase() === "plugin") {
						opts.plugin_name = pluginName;
					}

					await connection.execute(`begin ${createFileApi} end;`, opts);
